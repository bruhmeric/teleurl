...
12. **`RuntimeError: Timeout context manager should be used inside a task`** (the real cause, not the per-request `timeout=`): the global `aiohttp.ClientSession` in `utils/shared.py` was bound to whatever event loop was running when it was first created. The Mini App's `/api/formats` endpoint runs on Uvicorn's loop, so the session got bound to Uvicorn's loop. But the `/api/download` endpoint schedules the actual download onto the bot's loop via `asyncio.run_coroutine_threadsafe(...)`. When the download coroutine then tried to reuse the cached session, `aiohttp`'s internal `TimerContext.__enter__` called `asyncio.current_task(session._loop)` — which returned `None` because we were on the bot's loop, not Uvicorn's loop. Fixed by making `get_http_session()` loop-aware: it tracks the bound loop and recreates the session on the current running loop whenever the cached session's loop doesn't match. The old session is closed via `run_coroutine_threadsafe` on its own loop.
13. **Instagram / TikTok / Facebook downloads failing or timing out**: yt-dlp now requires a logged-in `cookies.txt` for these platforms (since ~2023). Without cookies, yt-dlp spent 10–30 s hitting the login wall before failing — making the bot feel slow — and the fallback chain never even tried cobalt because Instagram wasn't in `COBALT_DOMAINS`. Fixed by: (a) adding Instagram/TikTok/Facebook/Pinterest/Snapchat/SoundCloud/VK/Tumblr to `COBALT_DOMAINS`, (b) introducing a `COBALT_FIRST_DOMAINS` short-circuit in `download_url()` that tries cobalt BEFORE yt-dlp for cookie-gated social sites, (c) adding a cobalt shortcut in `fetch_ytdlp_formats()` so `/api/formats` returns a single placeholder format immediately (no 30 s yt-dlp wait), and (d) adding fast-fail yt-dlp opts (`socket_timeout=12`, `retries=1`) for cookie-gated sites when no cookies file is present, so the yt-dlp fallback path is also fast.
14. **Dead-code sniffer fallback unreachable**: when yt-dlp failed and `is_cobalt_url(url)` was False (e.g. Instagram before fix #13), the entire `if is_cobalt_url(url):` block was skipped — including the unreachable `else` branch that called `fetch_link_api()`. So the sniffer fallback never ran for non-cobalt URLs. Fixed by restructuring the `except Exception as ytdlp_err:` block: now it always tries cobalt (if available), then always tries the sniffer, then raises a combined error with all the underlying failures concatenated.
15. **Single cobalt instance going down (404 "No active service")**: the previous default `COBALT_API_URL` pointed to a Koyeb free-tier instance that sleeps after ~5 min of inactivity. When it slept, cobalt returned a 404 "No active service" page and the bot had no fallback. Fixed by (a) switching the default to `https://dwnld.nichind.dev` — a public meta-cluster that internally tries ~17 cobalt instances and returns the first successful result, and (b) adding a new `COBALT_API_FALLBACKS` env var (comma-separated list) that `download_cobalt()` now iterates through in order. The first endpoint that returns a valid download URL wins; failures are logged but don't abort the chain.
16. **Instagram 429 (Too Many Requests)**: Render's IP range is heavily rate-limited by Instagram, so yt-dlp's direct Instagram extractor fails with HTTP 429 even when cookies are provided. The fix is to never let yt-dlp try Instagram first — the `COBALT_FIRST_DOMAINS` short-circuit (fix #13) already routes Instagram through cobalt, which uses its own server-side IP pool that isn't rate-limited the same way. If you still see 429s, set `COBALT_API_FALLBACKS` to multiple cobalt URLs so the bot can rotate.
